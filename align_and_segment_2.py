...
def main(args):
    ...
    with open( os.path.join(args.outdir,args.name + "_manifest.json"), "w") as f:
        for i, t in enumerate(transcripts):
            span = spans[i]
            seg_start_idx = span[0].start
            seg_end_idx = span[-1].end

            output_file = os.path.join(args.outdir,args.name + "_segment_" + str(i) + ".flac")
